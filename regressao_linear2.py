...
Original file is located at
    https://colab.research.google.com/drive/1MsjMmSOVoKOGx0OiJGK0vBc4_h7raEJt
...
dados.describe().round(2) #quantidade; média; desvio padrão; valor minimo; 1º quartil; mediana; 3º quartil; valor máximo
...
ax.figure.set_size_inches(8,4)
ax.set_title('Distribuição de frequências', fontsize=12)
ax.set_ylabel('Consumo (Litros)', fontsize=12)
...
ax = sns.pairplot(dados, y_vars='consumo', x_vars=['temp_min', 'temp_media', 'temp_min', 'chuva','fds'])
ax.fig.suptitle('Dispersão entre as variáveis', fontsize=20, y=1.2)
ax

ax = sns.pairplot(dados, y_vars='consumo', x_vars=['temp_min', 'temp_media', 'temp_min', 'chuva','fds'], kind='reg')
ax.fig.suptitle('Dispersão entre as variáveis', fontsize=20, y=1.2)
ax
...
modelo.predict(entrada)[0] #Para um dia de 30.5°, 12,2 mm de chuva e sem ser um fim de semana esta previsto o consumo de 26.094 litros de cerveja
...
modelo.intercept_ #consumo medio independente das outras variaveis
...
modelo.coef_ #temp max, chuva, fds - efeito dessas variaveis no y/consumo
...
#termo de erro - variavel dependente - as previsões
...
eqm = metrics.mean_squared_error(y_test, y_previsto).round(2) #eqm e reqm tem que ser menores
reqm = np.sqrt(metrics.mean_squared_error(y_test, y_previsto)).round(2)
r2 = metrics.r2_score(y_test, y_previsto).round(2) #tem que ser maior
